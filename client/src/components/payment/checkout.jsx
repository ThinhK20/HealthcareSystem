...
function CheckOut() {
  ...
  const getData = async () => {
    const data = await authFetch.get("/Payments/GetPaymenOfUser/2");
    setData(data.data);
                  ...
                  <div className="overflow-hidden">
                    <table className="min-w-full mb-0">
                      ...
                      <tbody>
                        {data.map((item, index) => {
                          if (
                            Date.parse(item.createdDate) <= Date.now() &&
                            Date.parse(item.expirationDate) >= Date.now() &&
                            item.status == false
                          ) {
                            setCheck(1);
                            return (
                        ...
                        {check == 0 && (
                          <tr key={1} className={"border-b bg-gray-100"}>
                            <td
