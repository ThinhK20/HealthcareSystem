...
const CustomersPayment = () => {
  ...
  const GetPayments = () => {
    Axios.get(`https://localhost:44384/api/Payments/GetAllPaymentRequests`).then((response) => {
      SetPayments(response.data.filter(x => x.customerRequest.accountId==acc))
      setStagaPayment(response.data.filter(x=>x.customerRequest.accountId==acc));
  ...
  const filteredPayments = stagePayment.filter((item) => {
    ...
    const searchLower = typeof searchValue === 'string' ? searchValue.toLowerCase() : '';
    const matchesSearch =
